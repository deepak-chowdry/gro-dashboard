...
export default function IndividualGreivancePage({ greivanceId }: { greivanceId: string }) {
    const [activeTab, setActiveTab] = useState("overview")
    const [details, setDetails] = useState<GrievanceDetails | undefined>(undefined)
    ...
    const fetchGreivanceDetails = useCallback(async () => {
        ...
        try {
            ...
            const response = await fetch(`/api/grievances-details?id=${greivanceId}`, {
                method: "GET",
                headers: {
            ...
            if (!response.ok) {
                throw new Error(`Failed to fetch grievance details: ${response.statusText}`);
            }
            ...
            setError(error instanceof Error ? error.message : "Failed to load grievance details");
        } finally {
            setLoading(false);
        }
    }, [greivanceId]);
    ...
    const updateResolution = async () => {
        ...
        try {
            ...
            setError(error instanceof Error ? error.message : "Failed to update resolution")
        } finally {
            setUpdating(false)
        }
    };
            ...
            <div className="min-h-screen p-6">
                <Card className="max-w-md mx-auto mt-20">
                    <CardContent className="p-6 text-center">
                        ...
                        <h3 className="text-lg font-semibold mb-2">Error Loading Grievance</h3>
                    ...
                    <TabsContent value="overview" className="mt-6">
                        {loading ? (
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="flex items-start justify-between">
                                    <div>
                                        <h1 className="text-xl font-bold">{details?.grievance?.title}</h1>
                                    </div>
                                </div>
                                ...
                                <Card>
                                    ...
                                    <CardContent className="flex flex-col gap-3">
                                        <div className="flex flex-col md:flex-row md:items-center gap-2">
                                            ...
                                            {details?.grievance?.cpgrams_category && (
                                                <span className="text-sm flex flex-wrap items-center gap-1">
                                                    {details.grievance?.cpgrams_category?.split('>').map((part, idx, arr) => {
                                                        const colors = [
                                                            'text-red-400', 'text-blue-400', 'text-green-400', 'text-yellow-400',
                                                            'text-purple-400', 'text-pink-400', 'text-teal-400', 'text-orange-400',
                                                            'text-indigo-400', 'text-emerald-400', 'text-fuchsia-400', 'text-cyan-400',
                                                        ];
                                                        const color = colors[idx % colors.length];
                                                        return (
                                                            <span key={idx} className={color + ' font-normal'}>
                                                                {part.trim()}
                                                                {idx < arr.length - 1 && (
                                        ...
                                        <div className="flex flex-col md:flex-row md:items-center gap-2">
                                            <label className="font-semibold text-sm">Registered Date:</label>
                                            <p className="text-sm">{details?.grievance?.grievance_received_date ? formatDate(details.grievance.grievance_received_date) : '-'}</p>
                                        </div>
                                        ...
                                        <div className="flex flex-col md:flex-row md:items-center gap-2">
                                            ...
                                            <p className="text-sm capitalize">{details?.grievance?.status}</p>
                                        </div>
                                    </CardContent>
                                </Card>
                                        ...
                                        <div className="flex flex-col md:flex-row md:items-center gap-5 md:gap-10">
                                            ...
                                            {details?.user?.District && details?.user?.State && (
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <p className="text-sm">{details.user?.District}, {details.user?.State}</p>
                                                </div>
                                            )}
                                        </div>
                                    </CardContent>
                                            ...
                                            <div className="divide-y divide-gray-200">
                                                {documents.map((doc) => (
                                                    <div key={doc.id} className="p-4 hover:bg-gray-50 transition-colors">
                                                        <div className="flex items-center justify-between">
                                                            <div className="flex items-center gap-3">
                                                                <FileText className="size-5 text-blue-500 mb-2.5 hidden md:block" strokeWidth={1.5} />
                                                                <div className="space-y-1">
                                                                    <h3 className="font-medium text-sm text-gray-900">{doc.name}</h3>
